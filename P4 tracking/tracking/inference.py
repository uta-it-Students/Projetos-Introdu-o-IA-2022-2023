    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            ...
            if len(joined_factor.unconditionedVariables()) > 1:
                incrementally_marginalized_joint = eliminate(joined_factor, variable)
                current_factors_list.append(incrementally_marginalized_joint)

        full_joint = joinFactors(current_factors_list)
        query_conditioned_on_evidence = normalize(full_joint)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        sum_total = sum(self.values())
        for key in self.keys():
            if sum_total >0:
                self[key] = self[key]  / sum_total
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        while len(self.particles) < self.numParticles:
            #print pos
            pos = self.legalPositions[count]
            self.particles.append(pos)
            nPos = len(self.legalPositions) - 1
            if count < nPos:
                count += 1
            else:
                count = 0
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        product_cartesian = list(itertools.product(self.legalPositions, self.legalPositions))

        # Shuffle the list of positions in order to ensure an even placement of particles
        random.shuffle(product_cartesian)

        for particle in range(self.numParticles):
            position = product_cartesian[particle % len(product_cartesian)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        weighted_particles = DiscreteDistribution()
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                ...
                if (ghost, particle[ghost]) not in weights:
                    # P(observation | pacmanPosition, particle, jailPosition)
                    weight = self.getObservationProb(observation[ghost], pacman_position,
                                                     particle[ghost], jailPosition)
                    weights[(ghost, particle[ghost])] = weight

                total_weight *= weights[(ghost, particle[ghost])]

            weighted_particles[particle] += total_weight

        # If all particles receive zero weight, reinitialize
        if weighted_particles.total() == 0:
            self.initializeUniformly(gameState)
            return

        # Sample numParticles particles from weighted_particles distribution
        new_particles = [weighted_particles.sample() for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            ghos_previous_positions = oldParticle

            if ghos_previous_positions not in dists_new_Pos:
                dists_new_Pos[ghos_previous_positions] = []
                for ghost in range(self.numGhosts):
                    newPosDist = self.getPositionDistribution(gameState,
                        ghos_previous_positions, ghost, self.ghostAgents[ghost])
                    dists_new_Pos[ghos_previous_positions].append(newPosDist)

            for ghost in range(self.numGhosts):
                newParticle[ghost] = dists_new_Pos[ghos_previous_positions][ghost].sample()
            """*** END YOUR CODE HERE ***"""
